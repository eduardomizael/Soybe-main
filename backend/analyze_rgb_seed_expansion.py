...
def _is_target_job(job: dict[str, Any]) -> bool:
...
def _candidate_rows(run_files: list[Path]) -> list[dict[str, Any]]:
    ...
    for run_file in run_files:
        ...
        for entry in run.get("jobs", []):
            ...
            if entry.get("status") != "success" or not _is_target_job(job):
                continue
...
def _summary_rows(rows: list[dict[str, Any]], samples: int) -> list[dict[str, Any]]:
...
def _paired_values(rows: list[dict[str, Any]], metric_key: str) -> dict[str, np.ndarray]:
    lookup = {
...
def _test_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for metric, metric_key in [("accuracy", "acuracia"), ("macro_f1", "macro_f1")]:
        values = _paired_values(rows, metric_key)
...
def main() -> int:
    ...
    rows = _dedupe(_candidate_rows(run_files))
    ...
    _write_csv(args.output_dir / "rgb_seed_summary.csv", _summary_rows(rows, args.bootstrap_samples))
    _write_csv(args.output_dir / "rgb_seed_statistical_tests.csv", _test_rows(rows))
    _write_csv(args.output_dir / "rgb_seed_representative_runs.csv", _representative_rows(rows))
